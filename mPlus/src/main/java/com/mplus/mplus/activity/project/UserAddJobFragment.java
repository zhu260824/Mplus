...
public class UserAddJobFragment extends BaseFragment {
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
		...
		list_empty_text.setText("您暂时还未参加项目	");
			...
			mPullToRefreshListView.setOnRefreshListener(new OnRefreshListener2<ListView>() {
				...
				public void onPullUpToRefresh(PullToRefreshBase<ListView> refreshView) {
					String label = DateUtils.formatDateTime(mActivity,System.currentTimeMillis(),
							DateUtils.FORMAT_SHOW_TIME | DateUtils.FORMAT_SHOW_DATE | DateUtils.FORMAT_ABBREV_ALL);
					refreshView.getLoadingLayoutProxy().setLastUpdatedLabel(label);
					Page page=UserAddJobListPaser.GetInstance(1).getPage();
					if (page==null){
						page=new Page();
						page.totalcount=1;
						page.pagenum=1;
					...
					if (page.currpage<totalpage) {
						...
						RequestManger.GetUserAddJob(mActivity, userid, currpages, 10, new Listener<RequestCall>() {
							...
							public void onResponse(RequestCall response) {
								...
								if (response.getParser().getResultSuccess()){
									if (pAdapter!=null) 
										pAdapter.notifyDataSetChanged(UserAddJobListPaser.GetInstance(1).getContents());
								}
							}}, errorListener);
					}else {
	...
	private void OnRefresh() {
		RequestManger.GetUserAddJob(mActivity, userid, 1, 10, new Listener<RequestCall>() {
			...
			public void onResponse(RequestCall response) {
				if (mPullToRefreshListView!=null && mPullToRefreshListView.isRefreshing()) 
					mPullToRefreshListView.onRefreshComplete();
				if (response.getParser().getResultSuccess()){
					pAdapter=new UserAddProjectAdapter(mActivity,UserAddJobListPaser.GetInstance(1).getContents());
