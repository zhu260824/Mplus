...
* @date       2015年8月26日
*/
public class UploadPicturesActivity extends BaseActivity implements OnClickListener {
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {  
		...
		if (requestCode==PHOTO_REQUEST_GALLERY) {
	        ...
	        crop(FileUtils.getUriPath(UploadPicturesActivity.this, uri));
			...
			if (FileUtils.isExternalStorageAvaliable()) {
                ...
                Toast.makeText(UploadPicturesActivity.this, "未找到存储卡，无法存储照片！", Toast.LENGTH_SHORT).show();
                return;
            }
			...
			UpLoadPic(userid, "png",version, new RequestCallBack<String>() {
				@Override
				public void onStart() {
					super.onStart();
				...
				public void onSuccess(ResponseInfo<String> arg0) {
					...
					try {
						...
						Toast.makeText(UploadPicturesActivity.this, "图片上传成功", Toast.LENGTH_SHORT).show();
						if (baseParser.getResultSuccess()) {
							String uri=new JSONObject(baseParser.getData()).getString("portraitpath");
							String version=new JSONObject(baseParser.getData()).getString("version");
							Intent backdata=new Intent();
							backdata.putExtra("picurl", uri);
							backdata.putExtra("version", version);
						...
						Toast.makeText(UploadPicturesActivity.this,arg0.result, Toast.LENGTH_SHORT).show();
						e.printStackTrace();
					}
				}
				...
				public void onFailure(HttpException arg0, String arg1) {
					...
					FileUtils.deleteFile(filename);
					FileUtils.deleteFile(camerpath);
					Toast.makeText(UploadPicturesActivity.this, arg1, Toast.LENGTH_SHORT).show();
				}
			});
		}
    ...
    private void crop(String filePath) {
        Intent intent = new Intent(UploadPicturesActivity.this,CropImageActivity.class);
        intent.putExtra(CropImageActivity.PHOTOURI, filePath);
        intent.putExtra(CropImageActivity.RATIOTYPE, CropImageActivity.RATIO_1_1);
	...
	public void camera() {
        ...
        // 判断存储卡是否可以用，可用进行存储
        if (FileUtils.isExternalStorageAvaliable()) {
        	camerpath=FileUtils.getAppSaveImageOnSDCard(UploadPicturesActivity.this);
	...
	private void UpLoadPic(String userid,String suffix,String version,RequestCallBack<String> reCallBack) {
...
//		params.addQueryStringParameter("name", "value");
		...
		http.send(HttpMethod.POST,AppUtils.getMetaValue(UploadPicturesActivity.this,"APP_URL")+UrlManger.UPLOADFILE,params,reCallBack);
	}
}
