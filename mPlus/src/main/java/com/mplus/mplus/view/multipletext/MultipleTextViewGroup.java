package com.mplus.mplus.view.multipletext;
...
public class MultipleTextViewGroup extends RelativeLayout {
	...
	public MultipleTextViewGroup(Context context, AttributeSet attrs) {
        ...
        TypedArray array = context.obtainStyledAttributes(attrs, R.styleable.MultipleTextViewGroup);
	...
	private void setTextViewsTrue(List<String> dataList) {
    	LogTool.showArrayListILog(dataList);
...
//            tv.setTag(i);// 标记position
            tv.setOnClickListener(new OnClickListener() {
                ...
                public void onClick(View v) {
                    if (listener != null) {
                        listener.onMultipleTVItemClick(v,((TextView)v).getText().toString());
                    }
                }
            });
    ...
    public interface OnMultipleTVItemClickListener {
        void onMultipleTVItemClick(View view,String textContent);
    }
    
    public void setTextViews(final List<String> dataList ,final List<String> selcedList) {
        if (layout_width >= 0) {
        	 if (selcedList!=null && selcedList.size()>=1) {
        		 setTextViewsTrue(dataList,selcedList);
 			}else {
 				 setTextViewsTrue(dataList);
			}
        } else {
    ...
    @SuppressLint("UseSparseArrays")
	private void setTextViewsTrue(List<String> dataList,List<String> selcedList) {
        ViewGroup parentView = (ViewGroup) getParent();
        parentView.getWidth();
        if (dataList == null || dataList.size() == 0) {
            return;
        ...
        for (int i = 0; i < dataList.size(); i++) {
            TextView tv = new TextView(context);
            String tcontent=dataList.get(i);
            tv.setText(tcontent);
            tv.setTextSize(textSize);
            if (textBackground != -1)
                tv.setBackgroundResource(textBackground);
            tv.setTextColor(textColor);
            for (String text : selcedList) {
				if (text.equals(tcontent)) {
					tv.setBackgroundResource(R.drawable.bg_text_backgroup_blue);
					tv.setTextColor(context.getResources().getColorStateList(R.color.text_blue_color));
					tv.setTag(true);
				}
			}
            tv.setGravity(Gravity.CENTER);
    ...
    public void setTextViewsNocheck(final List<String> dataList ) {
    	  if (layout_width >= 0) {
  				setTextViewsTrue(dataList);
         } else {
             ViewTreeObserver viewTreeObserver = getViewTreeObserver();
